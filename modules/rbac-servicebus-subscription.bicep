...
resource namespace 'Microsoft.ServiceBus/namespaces@2021-06-01-preview' existing = {
  ...
  resource topic 'topics' existing = {
    name: topicName
    resource subscription 'subscriptions' existing = {
...
resource roleAssignment 'Microsoft.Authorization/roleAssignments@2020-08-01-preview' = [for roleId in roleIds: {
  name: guid(subscription().subscriptionId, namespaceName, topicName, subscriptionName, roleId, principalId)
  scope: namespace::topic::subscription
  properties: {
